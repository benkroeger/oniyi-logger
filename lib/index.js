...
function Logger(label, options) {
  ...
  logLevels.forEach((logLevel) => {
    ...
    self[logLevel.label.toLowerCase()] = function writeLevel(...args) {
      self.write(util.format(self.template, logLevel.label, util.format.apply(util, args)), sink);
    };
  });
  ...
  if (self.label && (loggerOptions.debug || process.env.NODE_DEBUG && re.test(process.env.NODE_DEBUG))) {
    self.debug = function debug(...args) {
      self.write(util.format(self.template, 'DEBUG', util.format.apply(util, args)), self.errSink);
      return self;
    };
  }
}
